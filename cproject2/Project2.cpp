...
/* use these two macros and the function functions if you want --
 * not required */
 ...
 * [Summary] Moves pointer back three locations and checks if signature
 * matches. Also checks to make sure that s is not null. */
 ...
 * the characters from 'src' and return a pointer to the first character of 
 * actual string data. */
 ...
 * [Summary] Calculates the length and keeps track 
 *     of length and capacity in metadata. */
...
/* the parameter 'utstr' must be a utstring. 
 * Find the length of this string by 
 * accessing the meta-data and return that length */
 ...
 * append the characters from suffix to the string s. Do not allocate any 
 * additional storage, and only append as many characters as will actually 
 * fit in s. Update the length meta-data for utstring s
 * and then return s */
...
char* utstrcat(char* s, const char* suffix) 
{
    ...
    while((numCharsAdded < suffixLength) && 
            (strWithData->length < strWithData->capacity))
    {
 ...
 * overwrite the characters in dst with the characters from src. Do not 
 * overflow the capacity of dst. For example, if src has five characters 
 * and dst has capacity for 10, then copy all five characters. 
 * However, if src has ten characters and dst only has capacity for 8,
 * then copy only the first 8 characters. Do not allocate any additional 
 * storage, do not change capacity. Update the length meta-data for dst 
 * and then return dst */
 ...
 * [Summary] Has similar behaviour to utstrcat. Copies as many 
 *     characters as will fit from src into dst. */
char* utstrcpy(char* dst, const char* src) 
{
    ...
    /* Make length 0, then call cat. Copies the string,
     * since cat adds from index 0. */
 ...
 * the chunk, note that the start of
 ...
 * if s already has capacity equal to or larger than new_capacity, then return 
 * s. if s has less capacity than new_capacity, then allocate new storage with
 * sufficient space to store new_capacity characters (plus a terminating zero),
 * copy the current characters from s into this new storage. Update the 
 * meta-data to correctly describe new new utstring you've created, deallocate s
...
char* utstrrealloc(char* s, uint32_t new_capacity) 
{
    ...
    /* If s is a utrstring and has less space than new_capacity, 
     * then reallocate and update capacity */

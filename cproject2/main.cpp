...
void testStage5(){
    ...
    uint32_t len=0;
    while(compareString[len]!='\0') len++;

    char* p=utstrdup(compareString);
    

    if(strcmp(compareString, p) || utstrlen(p) != len){
        puts("You messed up utstrdup.");
    }

    p=utstrrealloc(p, 10);

    if(strcmp(compareString, p)){
        puts("You messed up realloc.");
    }

    p=utstrrealloc(p, len * 2);
    p=utstrcat(p, compareString);

    if(strcmp(compareString, p+len)){
        puts("You messed up realloc or cat. This time with cap>size");
    }

    p=utstrcpy(p, helloworld);

    if(strcmp(p, helloworld)){
        puts("Copy function fail.");
    }
